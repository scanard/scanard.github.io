...
document.addEventListener('click', function(event) {
    var becherPop = document.getElementById('becher_pop');
    var becher = document.querySelector('.becher');
    ...
    if (becherPop.classList.contains('visible') && !becherPop.contains(event.target) && !becher.contains(event.target)) {
        becherPop.classList.remove('visible');
    }
...
/// Indice
const indices = [
    "Indice 1 : Il faut essayer de coder la suite de lettre grâce à au tableau et tenter de retrouver les coordonnées de base.",
    "Indice 2 : La clé donnée doit être utilisée pour diviser une ligne des coordonnées pour trouver les coordonnées initiales.",
];
...
document.getElementById('verifierButton').addEventListener('click', function() {
    ...
    if (messageInput.toUpperCase() === 'ARTICHAUT') {
        startDialogSequence()
    } else {
...
closeAllBtns.forEach((btn) => {
    ...
    window.location.href = `../salle 6/salle6.html?playerName=${encodePlayerName}&totalScore=${encodeTotalScore}`;
    });
});
...
/// Cliquable 1
function openBecher() {
    document.getElementById('becher_pop').classList.add('visible');
};
