...
/// Indice
const indices = [
    "Indice 1 : E est la lettre la plus courante de l'alphabet",
    "Indice 2 : Il est possible de déchiffrer le code de César sans connaitre la clef. Analysez attentivement les occurences de chaques lettres",
];
...
document.getElementById('verifierButton').addEventListener('click', function() {
    ...
    if (messageInput.toUpperCase() === 'GEORGES PEREC A ECRIT UN LIVRE SANS LA LETTRE E LA DISPARITION') {
        currentDialog = 1;
        startDialogSequence();
    } else {
...
closeAllBtns.forEach((btn) => {
    ...
    window.location.href = `../salle 7/salle7.html?playerName=${encodePlayerName}&totalScore=${encodeTotalScore}`;
    });
});
