...
/// Indice
const indices = [
    "Indice 1 : Cette suite de lettres ressemble drôlement à celle de la dernière pièce.",
    "Indice 2 : Essayons d'appliquer César à nouveau mais avec les différents numéros.",
];
...
document.getElementById('verifierButton').addEventListener('click', function() {
    ...
    if (messageInput.toUpperCase() === 'SIMPLE COMME BONJOUR') {
        currentDialog = 1;
        startDialogSequence();
    } else {
...
closeAllBtns.forEach((btn) => {
    ...
    window.location.href = `../salle 4/salle4.html?playerName=${encodePlayerName}&totalScore=${encodeTotalScore}`;
    });
});
...
// Fermer le dialogue lorsqu'on clique en dehors du contenu du dialogue
...
/// Cliquable 1 : image
document.getElementById('annuaire').addEventListener('click', function() {
    document.getElementById('annuaire_pop').classList.add('visible');
    currentDialog = 0;
    startDialogSequence();
});
...
document.addEventListener('click', function(event) {
    var postitPop = document.getElementById('annuaire_pop');
    var postit = document.getElementById('annuaire');
